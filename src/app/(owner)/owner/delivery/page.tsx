...
import { getAllSettlements, getSettlementLedgers } from "@/services/money";
...
export default async function DeliveryPage() {
  ...
  const [platforms, settlements, ledgers] = await Promise.all([
    getPlatforms(db),
  ...
  const deliveryLedgers = ledgers.filter((l) => l.channel === "delivery");

  return (
    <DeliveryDashboard
      platforms={platforms}
      settlements={deliverySettlements}
      ledgers={deliveryLedgers}
    />
  );
}
