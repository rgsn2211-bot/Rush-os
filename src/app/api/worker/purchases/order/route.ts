...
import { orderWorkerPurchase } from "@/services/purchases";

/** Worker logs an order (status 'ordered') — cost-free, no review gate. */
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await orderWorkerPurchase(db, parsed.data, authUser.id);
