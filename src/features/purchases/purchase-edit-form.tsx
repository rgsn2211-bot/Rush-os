...
/**
 * Edit an existing purchase. Reuses the receive-style layout, pre-filled from
 * the current record. Payment fields are hidden for 'ordered' purchases (they
 * are never paid). Saving reverses the old effects and re-applies the new ones.
 */
export function PurchaseEditForm({
  ...
  const [paidMethod, setPaidMethod] = useState<"cash" | "bank">(
    purchase.paidMethod ?? "cash",
  );
  const [dueDate, setDueDate] = useState(purchase.dueDate ?? "");
  const [showItemPicker, setShowItemPicker] = useState(false);
  const [lines, setLines] = useState<EditLine[]>(() =>
    items.map((pi) => {
      const item = itemMap.get(pi.inventoryItemId);
      return {
        ...
        purchaseQty: pi.purchaseQty,
        unitCostBhd: String(filsToBhd(pi.unitCostFils)),
        expiryDate: pi.expiryDate ?? undefined,
      };
    }),
  );

  const canPay = purchase.status !== "ordered";

  function lineTotalFils(line: EditLine): number {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    if (lines.length === 0) {
      setError("A purchase needs at least one item.");
      return;
    }
    setLoading(true);
    ...
    const body = {
      ...
      isPaid: canPay ? isPaid : false,
      paidMethod: canPay && isPaid ? paidMethod : undefined,
      dueDate: canPay && !isPaid && dueDate ? dueDate : undefined,
      items: lines.map((l) => ({
    ...
    router.push(`/owner/purchases/${purchase.id}`);
    router.refresh();
  }
    ...
    <form onSubmit={handleSubmit}>
      <div className="grid items-start gap-5 lg:grid-cols-[1.5fr_1fr]">
          ...
          <Card className="p-0">
            ...
            {lines.length > 0 ? (
              ...
              <div className="text-ink-3 px-5 py-8 text-center text-sm">
                No items. Use &ldquo;Add item&rdquo; to add one.
              </div>
            )}
        ...
        <Card className="lg:sticky lg:top-20">
          <CardContent>
            <h3 className="mb-2 text-[15px] font-bold">Save changes</h3>
            <p className="text-ink-3 mb-4 text-[13px] leading-relaxed">
              {purchase.status === "approved"
                ? "This purchase already landed stock. Saving re-applies the corrected quantities and costs to on-hand stock, average cost, and any payment."
            ...
            <Button
              ...
              full
              className="mt-2"
              onClick={() => router.push(`/owner/purchases/${purchase.id}`)}
            >
              Cancel
            </Button>
          </CardContent>
